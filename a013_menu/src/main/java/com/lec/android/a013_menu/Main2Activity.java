...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerForContextMenu(tvCtxMenu);  //뷰객체(위젯.. 등) 에 ContextMenu 를 등록.
    ...
    // onCreateContextMenu()
    // 컨텍스트 메뉴가 생성될때마다 호출되는 콜백 메서드(롱클릭시)
    @Override
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        super.onCreateContextMenu(menu, v, menuInfo);
        Log.d("myapp", "onCreateContextMenu()");

        // 각각의 뷰-View v (실제 메뉴) 마다 다르게 메뉴 작동하도록 설정하기 가능하다.
        switch (v.getId()){ // id 값 넘어온다.
            case R.id.tvCtxMenu:
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item) {
        ...
        int id = item.getItemId(); // 메뉴 아이템의 아이디 값
