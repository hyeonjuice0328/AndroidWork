...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 방법3
        // 메인 스레드 의 handler 를 외부 클래스에서 넘겨 받아
    ...
    Handler handler1 = new Handler() { // 익명클래스 안에서 handleMessage 를 override
        ...
        // Main 에서 생성한 Runnable
        // 방법2
        Handler handler2 = new Handler();

        // 방법3
        Handler handler3 = new Handler(){ //익명클래스

            @Override
            public void handleMessage(@NonNull Message msg) {
                if(msg.what == 0 ){
                    // 그 메세지를 통해 받은 값을 메인 UI 에 출력
    ...
    // 작업 스레드 ( 내부 클래스가 아니기 때문에 ... 조건이 다르다)
class  BackThread3 extends Thread{

        int backValue = 0;
        Handler handler;

        //외부로부터  handler 받아서 내부에서 셋팅
        ...
        public void run() {
            while (true){
                ...
                msg.what = 0; // 메세지 아이디를 0으로 셋팅
                msg.arg1 = backValue; //  msg 에 정수값을 넣을 수 있다.
...
// 방법4 :  메인 스레드의 Handler를 직접 사용할 수 없는 분리된 작업 스레드
class  BackThread4 extends  Thread{
    int backValue = 0;
    Handler handler;

    BackThread4(Handler handler) {this.handler = handler;}  //여기까지는 방법3과 같음
